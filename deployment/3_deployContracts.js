...
async function main() {
  ...
  for (const parameterName of mandatoryDeploymentParameters) {
    if (
      deployParameters[parameterName] === undefined ||
      deployParameters[parameterName] === ""
    ) {
  ...
  const cdkValidiumDeployerContract = CDKValidiumDeployerFactory.attach(
    cdkValidiumDeployerAddress
  );

  // check deployer is the owner of the deployer
  if (
    (await deployer.provider.getCode(cdkValidiumDeployerContract.address)) ===
    "0x"
  ) {
    throw new Error("cdkValidium deployer contract is not deployed");
  }
  expect(deployer.address).to.be.equal(
    await cdkValidiumDeployerContract.owner()
  );
  ...
  if (!ongoingDeployment.verifierContract) {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(ongoingDeployment, null, 1)
    );
  } else {
    console.log(
      "Verifier already deployed on: ",
      ongoingDeployment.verifierContract
    );
    const VerifierRollupFactory = await ethers.getContractFactory(
      "FflonkVerifier",
      deployer
    );
  ...
  const dataCallAdmin = proxyAdminFactory.interface.encodeFunctionData(
    "transferOwnership",
    [deployer.address]
  );
  ...
  const deployTransactionBridge =
    PolygonZkEVMBridgeFactory.getDeployTransaction().data;
  const dataCallNull = null;
  // Mandatory to override the gasLimit since the estimation with create are mess up D:
  const overrideGasLimit = ethers.BigNumber.from(5500000);
  const [bridgeImplementationAddress, isBridgeImplDeployed] =
    await create2Deployment(
      cdkValidiumDeployerContract,
      salt,
  ...
  if (isBridgeImplDeployed) {
    ...
    console.log("#######################\n");
    console.log(
      "bridge impl was already deployed to:",
  ...
  const nonceProxyGlobalExitRoot =
    Number(await ethers.provider.getTransactionCount(deployer.address)) +
    nonceDelta;
  ...
  if (
    ...
    ongoingDeployment.cdkValidiumContract
  ) {
  ...
  const PolygonZkEVMBridgeContract =
    PolygonZkEVMBridgeFactory.attach(proxyBridgeAddress);

  if (isBridgeProxyDeployed) {
    console.log("#######################\n");
    console.log(
      "PolygonZkEVMBridge deployed to:",
      PolygonZkEVMBridgeContract.address
    );
  } else {
    console.log("#######################\n");
    console.log(
      "PolygonZkEVMBridge was already deployed to:",
      PolygonZkEVMBridgeContract.address
    );

    // If it was already deployed, check that the initialized calldata matches the actual deployment
    expect(precalculateGLobalExitRootAddress).to.be.equal(
      await PolygonZkEVMBridgeContract.globalExitRootManager()
    );
    expect(precalculateCDKValidiumAddress).to.be.equal(
      await PolygonZkEVMBridgeContract.polygonZkEVMaddress()
    );
  }
  ...
  for (let i = 0; i < attemptsDeployProxy; i++) {
    try {
      ...
      console.log(
        "upgrades.deployProxy of cdkDataCommitteeContract ",
        error.message
      );
    }
    ...
    if (i + 1 === attemptsDeployProxy) {
      throw new Error(
        "cdkDataCommitteeContract contract has not been deployed"
      );
    }
  }
  ...
  if (setupEmptyCommittee) {
    ...
    await expect(
      cdkDataCommitteeContract.connect(deployer).setupCommittee(0, [], [])
    )
  ...
  if (!ongoingDeployment.PolygonZkEVMGlobalExitRoot) {
    for (let i = 0; i < attemptsDeployProxy; i++) {
      try {
        ...
        console.log(`attempt ${i}`);
        console.log(
          "upgrades.deployProxy of PolygonZkEVMGlobalExitRoot ",
          error.message
        );
      }
      ...
      if (i + 1 === attemptsDeployProxy) {
        throw new Error(
          "PolygonZkEVMGlobalExitRoot contract has not been deployed"
        );
      }
    }
    ...
    console.log("#######################\n");
    console.log(
      "PolygonZkEVMGlobalExitRoot deployed to:",
      PolygonZkEVMGlobalExitRoot.address
    );
    ...
    console.log("#######################\n");
    console.log(
      "PolygonZkEVMGlobalExitRoot already deployed on: ",
      ongoingDeployment.PolygonZkEVMGlobalExitRoot
    );
    ...
    expect(PolygonZkEVMBridgeContract.address).to.be.equal(
      await PolygonZkEVMBridgeContract.bridgeAddress()
    );
    expect(precalculateCDKValidiumAddress).to.be.equal(
      await PolygonZkEVMBridgeContract.rollupAddress()
    );
  }
  ...
  if (!ongoingDeployment.cdkValidiumContract) {
    for (let i = 0; i < attemptsDeployProxy; i++) {
      try {
        ...
        console.log(`attempt ${i}`);
        console.log(
          "upgrades.deployProxy of cdkValidiumContract ",
          error.message
        );
      }
    ...
    console.log("#######################\n");
    console.log(
      "cdkValidiumContract deployed to:",
      cdkValidiumContract.address
    );
    ...
    if (cdkValidiumOwner !== deployer.address) {
      await (
        await cdkValidiumContract.transferOwnership(cdkValidiumOwner)
      ).wait();
    }

    deploymentBlockNumber = (await cdkValidiumContract.deployTransaction.wait())
      .blockNumber;
  } else {
    ...
    cdkValidiumContract = CDKValidiumFactory.attach(
      ongoingDeployment.cdkValidiumContract
    );

    console.log("#######################\n");
    console.log(
      "cdkValidiumContract already deployed on: ",
      ongoingDeployment.cdkValidiumContract
    );
  ...
  console.log(
    "PolygonZkEVMGlobalExitRootAddress:",
    await cdkValidiumContract.globalExitRootManager()
  );
  ...
  console.log(
    "PolygonZkEVMBridgeContract:",
    await cdkValidiumContract.bridgeAddress()
  );
  ...
  console.log(
    "trustedSequencer:",
    await cdkValidiumContract.trustedSequencer()
  );
  console.log(
    "pendingStateTimeout:",
    await cdkValidiumContract.pendingStateTimeout()
  );
  console.log(
    "trustedAggregator:",
    await cdkValidiumContract.trustedAggregator()
  );
  console.log(
    "trustedAggregatorTimeout:",
    await cdkValidiumContract.trustedAggregatorTimeout()
  );

  console.log("genesiRoot:", await cdkValidiumContract.batchNumToStateRoot(0));
  console.log(
    "trustedSequencerURL:",
    await cdkValidiumContract.trustedSequencerURL()
  );
  ...
  expect(
    await upgrades.erc1967.getAdminAddress(precalculateCDKValidiumAddress)
  ).to.be.equal(proxyAdminAddress);
  expect(
    await upgrades.erc1967.getAdminAddress(precalculateGLobalExitRootAddress)
  ).to.be.equal(proxyAdminAddress);
  expect(
    await upgrades.erc1967.getAdminAddress(proxyBridgeAddress)
  ).to.be.equal(proxyAdminAddress);
  ...
  if (proxyAdminOwner !== deployer.address) {
    ...
    expect(precalculateCDKValidiumAddress).to.be.equal(
      await timelockContract.cdkValidium()
    );
    ...
    console.log("transferProxyAdminOwnership to timelockContract");
    await upgrades.admin.transferProxyAdminOwnership(timelockContract.address);
  }

  if (committeeTimelock) {
    await (
      await cdkDataCommitteeContract.transferOwnership(timelockContract.address)
    ).wait();
  }
