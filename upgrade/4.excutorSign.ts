...
async function main() {
  ...
  const id = upgradeParams.id;
  const timestamp = await CDKValidiumTimelock.getTimestamp(id);
  if(timestamp.eq(0)) {
    throw new Error(`Upgrade not scheduled, id: ${id}`);
  }
